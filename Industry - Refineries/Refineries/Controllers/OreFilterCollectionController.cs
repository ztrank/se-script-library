namespace IngameScript
{
    ...
    partial class Program
    {
        public class OreFilterCollectionController : UserInterface.SelectionController<OreFilter, OreFilterCollection, OreFilterCollectionModel, OreFilterCollectionView>
        {
            ...
            /// <summary>
            /// Show the screen saver for the view.
            /// </summary>
            /// <param name="on">True, default, turns the screen saver on.</param>
            public void SetScreenSaver(bool on = true)
            {
                this.Model.ShowScreenSaver = on;
                this.Rerender();
            }
            ...
            public void SetSelected(List<MyInventoryItemFilter> filters, bool replace = false)
            {
                if (replace)
                {
            ...
            public void GetSelected(List<MyInventoryItemFilter> filters)
            {
                filters.AddRange(this.Model.Collection.Values.Where(f => f.IsSelected).Select(f => new MyInventoryItemFilter(f.DefinitionId, false)));
            }
        }
    }
}
